...
interface VoteCommentTypeCondition {
	commentId: number;
	userId: number;
}
...
const batchGetVoteCommentTypes = async (
	voteTypeConditions: VoteCommentTypeCondition[]
) => {
	const voteTypes = await UpvoteComment.findByIds(voteTypeConditions);
	return voteTypeConditions.map((voteTypeCondition) =>
		voteTypes.find(
			(voteType) =>
				voteType.commentId === voteTypeCondition.commentId &&
				voteType.userId === voteTypeCondition.userId
		)
	);
};
...
export const buildDataLoaders = () => ({
	...
	voteCommentTypeLoader: new DataLoader<
		VoteCommentTypeCondition,
		UpvoteComment | undefined
	>((voteTypeConditions) =>
		batchGetVoteCommentTypes(voteTypeConditions as VoteCommentTypeCondition[])
	),
});
